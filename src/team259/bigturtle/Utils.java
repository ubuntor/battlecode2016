...
public class Utils {
	...
	public static final int ARCHON_COVERED = 140808;
	...
	public static final int SCOUT_VISION = 23941;
	...
	public static final int TURRET_BUILT = 231413;
	...
	public static final int GUARD_PATROL = 128031;
...
//			System.out.println("" + i + " " + damageTaken);
			if(damageTaken == 0){
				bestDir = i;
				break;
			} else if(damageTaken < leastDamage){
				bestDir = i;
				leastDamage = damageTaken;
			}
		}
...
//		System.out.println();
	...
	public static double priority(RobotInfo r){
		double prio = 0;
		switch(r.type){
		case SOLDIER:
			prio = 2;
		...
		case ZOMBIEDEN:
			...
			break;
		case ARCHON:
			prio = Double.MAX_VALUE;
			break;
		case SCOUT:
			prio = 1e-9;
		}
		
		return prio / Math.sqrt(r.health);
	}
	
	public static boolean surrounded(RobotController rc) throws Exception{
		MapLocation currLoc = rc.getLocation();
		for(Direction dir : DIRS){
			RobotInfo nbor = rc.senseRobotAtLocation(currLoc.add(dir));
			if(nbor == null || !nbor.team.equals(rc.getTeam()) ||
					!(nbor.type.equals(RobotType.ARCHON) || nbor.type.equals(RobotType.TURRET)
							|| nbor.type.equals(RobotType.TTM) || nbor.type.equals(RobotType.SCOUT))){
				return false;
			}
		}
		return true;
