...
public class Utils {
	...
	public static final int ARCHON_HOME = 120983;
	
	//scout signaling
	public static final int SCOUT_SCOUT = 12983;
	public static final int SCOUT_PULL = 3000;
	...
	public static boolean safeMove(RobotController rc, MapLocation loc, RobotInfo[] enemies, RobotInfo[] zombies) throws Exception {
		...
		for (int i = 0; i <= 4; i++) {
			Direction dir = DIRS[(start + SEARCH[i]) % 8];
	...
	public static boolean safestMove(RobotController rc, MapLocation loc, RobotInfo[] enemies, RobotInfo[] zombies) throws Exception {
		...
		for (int i = 0; i < 8; i++) {
			...
			MapLocation tempLoc = rc.getLocation().add(dir);
			if (!rc.canMove(dir) && (rc.isLocationOccupied(tempLoc) || !rc.onTheMap(tempLoc)))
				continue;
			double damageTaken = 0;
			double tempRubble = rc.senseRubble(tempLoc);
			for(RobotInfo r : enemies){
				if (r.type.equals(RobotType.SOLDIER) && r.location.distanceSquaredTo(tempLoc) <= 13)
					damageTaken += 4;
				else if (r.type.equals(RobotType.GUARD) && r.location.distanceSquaredTo(tempLoc) <= 2)
			...
			if(damageTaken == 0){
				bestDir = i;
				break;
			} else if(i <= 4 && damageTaken < leastDamage)
				bestDir = i;
			else if(i <= 4 && damageTaken == leastDamage && tempRubble < rubble)
				bestDir = i;
			else if(i > 4 && damageTaken < leastDamage)
				bestDir = i;
			if((bestDir < 0) ||
					(rc.getType().equals(RobotType.ARCHON) && rc.senseParts(tempLoc) > rc.senseParts(currLoc.add(DIRS[bestDir]))))
				bestDir = i;
		...
		//Direction[] options = new Direction[]{toMove.rotateLeft(), toMove, toMove.rotateRight()};
		if(!rc.canMove(toMove)){
			rc.clearRubble(toMove);
			return false;
		}

		rc.move(DIRS[(start + 8+ SEARCH[bestDir]) % 8]);
		return true;
