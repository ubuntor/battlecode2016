        ...
        try {
            ...
            for(Signal s : sigs){
            	if(s.getTeam().equals(myTeam)){
            		...
            		if(message != null && message[0] == Utils.GUARD_PATROL){
            			int[] temp = Utils.unpack2(message[1]);
            			tlArchon = new MapLocation(temp[0], temp[1]);
            		}
            		if(message != null && message[0] == Utils.ACTIVATED_NEUTRAL && message[1] == rc.getID()){
                		neutral = true;
                		break;
                	}
            	}
...
//            	System.out.println("!!");
            ...
            patrol[0] = tlArchon.add(Direction.NORTH_WEST, 4);
            patrol[1] = tlArchon.add(Direction.NORTH_EAST, 4);
            patrol[2] = tlArchon.add(Direction.SOUTH_EAST, 4);
            patrol[3] = tlArchon.add(Direction.SOUTH_WEST, 4);
            if(numArchons > 1){
...
//            System.out.println(Arrays.toString(patrol));
        ...
        while (true) {
            try {
                ...
                RobotInfo[] hostile = rc.senseHostileRobots(currLoc, rc.getType().sensorRadiusSquared);
                if(hostile.length > 0){
                	defend(rc, attackLoc, hostile);
                } else {
...
//						System.out.println("" + rc.getID() + ": clearing rubble");
                		...
                		if(inWay == null){
                			if(rc.senseRubble(currLoc.add(moveDir)) / 150 > Math.random())
                				rc.clearRubble(moveDir);
...
//                	System.out.println(myTeam);
    ...
    private static void defend(RobotController rc, MapLocation attackLoc, RobotInfo[] hostile) throws Exception{
